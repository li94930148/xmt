...
- 复盘行动项
- 数据导出
- 抖音官方导出与作品数据接入；缺少同口径基线时显示不可用
- 抖音运营中心展示已采集的趋势和粉丝指标
- OpenAPI 覆盖部分 v1 接口，Webhook 与数据同步按实际路由提供
...
`agent/` 是独立的 Creator Agent 桌面端项目，不是主 Web 项目的运行依赖。当前支持 Windows 与 macOS；采集运行时、浏览器配置和安装包分别按平台构建与验证。当前自动同步以抖音官方导出为主。

在 macOS 上开发和运行主 Web 项目时：

- 不需要安装 `agent/` 下的依赖
- 不要复用 Windows 二进制文件或安装包
- macOS Agent 需使用对应平台的 Collector Runtime 与安装包，并验证浏览器和本机凭据状态
