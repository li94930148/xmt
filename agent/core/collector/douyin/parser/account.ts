...
const optionalNumber = (value: unknown) => value === undefined || value === null || value === '' ? null : Number.isFinite(Number(value)) ? Number(value) : null;
...
export function mergeAccountDomFallback(account:ReturnType<typeof parseAccount>,body:string){
  ...
  const count=(label:string)=>optionalNumber(body.match(new RegExp(`${label}\\s*([0-9.]+)(万)?`))?.slice(1).reduce((value,part,index)=>index===0?Number(part):part?Number(value)*10_000:value,0));
