...
export class ScraplingCreatorCollector {
  ...
  async collect(options: { collectionMode?: CollectionMode } = {}): Promise<CreatorSnapshot> {
    ...
    return { schema_version: 1, protocol_version: 1, agent_version: '2.12.0-agent', platform: 'douyin', source: 'local_creator_center', contract_version: '2.10.2', snapshot_id: crypto.randomUUID(), collection_mode: options.collectionMode || 'full_snapshot', collection_stats: { raw_response_count: Number(data.xhrResponses), aweme_candidate_count: works.length, normalized_success_count: works.length, rejected_count: 0, rejected_reasons: {}, page_count: Number(data.pages || 0), new_count: works.length }, collected_at: new Date().toISOString(), account: { uid: this.accountId, nickname: '', avatar: '', fans_count: null }, works, work_details: [], dashboard: {}, content_analysis: {}, fans: {}, raw: { api_map: [], captures: [] }, videos: works, operations: { last7Days: {}, last30Days: {}, trafficSources: [], contentPerformance: {} } };
  }
}
