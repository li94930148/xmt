...
import { encrypt, sign } from "../crypto/envelope.js";
...
async function responseJson(response: Response) {
  ...
  if (!response.ok)
    throw new Error(
      String(value.message || `请求失败 HTTP ${response.status}`),
    );
  return value;
}
export async function loginXmt(
  serverUrl: string,
  username: string,
  password: string,
) {
  ...
  return responseJson(response) as Promise<{ token: string; user?: unknown }>;
}
export async function register(
  ...
  const response = await fetch(
    `${serverUrl.replace(/\/$/, "")}/api/creator-agent/register`,
    {
      method: "POST",
      headers: {
...
export async function bind(
  ...
  const response = await fetch(
    `${serverUrl.replace(/\/$/, "")}/api/creator-agent/bind`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ binding_code: bindingCode, device }),
    },
  );
  return responseJson(response) as Promise<{
    agent_id: number;
...
export async function heartbeat(
  config: AgentConfig,
  agentToken: string,
  state: { deviceName: string; os: string; browserLoginStatus: string },
) {
  const browser = config.browserConfig;
  const response = await fetch(
    `${config.serverUrl.replace(/\/$/, "")}/api/creator-agent/heartbeat`,
    {
      method: "POST",
      headers: {
        "content-type": "application/json",
      ...
      body: JSON.stringify({
        ...
        compatibility_status: browser.compatibilityStatus || "not_tested",
      }),
    },
  );
...
export async function uploadCanonicalPayload(config: AgentConfig, agentToken: string, payload: Record<string, unknown>, agentVersion: string, collectedAt: string) {
  ...
  const clientPayloadBytes = Buffer.byteLength(serialized);
  if (clientPayloadBytes > 12 * 1024 * 1024)
    throw new Error("同步数据包超过 12MB 限制，请缩小采集范围后重试");
  const response = await fetch(
    `${config.serverUrl.replace(/\/$/, "")}/api/creator-agent/data-sync`,
    {
      method: "POST",
      headers: {
        "content-type": "application/json",
  ...
  const result = (await responseJson(response)) as {
    ...
    failed_count: number;
    modules: Record<string, "success" | "failed">;
  ...
  return { ...result, client_payload_bytes: clientPayloadBytes };
}
