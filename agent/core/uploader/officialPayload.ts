...
export function toOfficialExportPayload(snapshot: CreatorSnapshot, accountId: string) {
  ...
  const accountDailyMetrics = records(snapshot.official_data).flatMap(item => records((item.datasets as Record<string, unknown> | undefined)?.account_daily_metrics));
  ...
  const parserVersion = 'douyin-export-v3';
  ...
  return {
    ...
    source_files: files.map(file => ({
      file_type: 'official_export',
      dataset_type: String(file.datasetType || ''),
      period: file.period == null ? null : String(file.period),
      file_name: String(file.storedFilename || ''),
      sha256: String(file.sha256 || ''),
      size_bytes: Number(file.size || 0),
      downloaded_at: snapshot.collected_at,
    })),
    datasets: { content_metrics: contentMetrics, account_daily_metrics: accountDailyMetrics, income_metrics: incomeMetrics },
    quality,
  };
}
