...
class CreatorDatabase {
    ...
    knownContentIds() { return new Set(this.db.prepare('SELECT item_id FROM creator_works').all().map((row) => String(row.item_id))); }
    ...
    snapshotCounts() {
        ...
        return Object.fromEntries(tables.map((table) => [table, Number(this.db.prepare(`SELECT COUNT(*) count FROM ${table}`).get().count)]));
    }
    close() { this.db.close(); }
}
