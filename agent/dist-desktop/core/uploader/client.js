...
async function upload(config, agentToken, snapshot, options = {}) { const body = { agent_id: config.agentId, platform: config.platform, account_id: config.accountId, collected_at: snapshot.collected_at || new Date().toISOString(), data: (0, envelope_js_1.encrypt)((0, unifiedPayload_js_1.toUnifiedCreatorPayload)(snapshot, options), agentToken) }; body.signature = (0, envelope_js_1.sign)(body, agentToken); const response = await fetch(`${config.serverUrl.replace(/\/$/, '')}/api/creator-agent/data-sync`, { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${agentToken}` }, body: JSON.stringify(body) }); return responseJson(response); }
