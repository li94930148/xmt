...
async function initTables() {
  ...
  await db.execute(`CREATE TABLE IF NOT EXISTS creator_cover_assets (id INTEGER PRIMARY KEY AUTOINCREMENT,account_id INTEGER NOT NULL,platform_item_id TEXT NOT NULL,mime_type TEXT NOT NULL,sha256 TEXT NOT NULL,size_bytes INTEGER NOT NULL,bytes BLOB NOT NULL,created_at DATETIME DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,UNIQUE(account_id,platform_item_id),FOREIGN KEY(account_id) REFERENCES creator_platform_accounts(id) ON DELETE CASCADE)`);
