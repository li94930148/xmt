...
router.get('/monthly', authenticate, requireRole(['admin', 'director']), async (req, res) => {
  try {
...
router.get('/user', authenticate, requireRole(['admin', 'director']), async (req, res) => {
  try {
    const { month, year } = req.query;
    const current = getBeijingMonthYear();
    const targetMonth = parseInt(month as string) || current.month;
    ...
    const userStats = await queryAll(`SELECT 
                             u.name as user_name,
                             u.id as user_id,
...
router.get('/team', authenticate, requireRole(['admin', 'director']), async (req, res) => {
  try {
    const { month, year } = req.query;
    const current = getBeijingMonthYear();
    const targetMonth = parseInt(month as string) || current.month;
    ...
    const completedTopics = await queryOne(`SELECT COUNT(*) as count FROM topics 
    ...
    const overdueTopics = await queryOne(`SELECT COUNT(*) as count FROM topics 
                                 WHERE deadline < datetime('now', '+8 hours') AND status != 'completed'
                                 AND strftime('%Y', created_at) = ? AND strftime('%m', created_at) = ?`,
      [targetYear.toString(), targetMonth.toString().padStart(2, '0')]);
    
    const avgDays = await queryOne(`SELECT AVG(julianday(updated_at) - julianday(created_at)) as avg_days 
                           FROM topics WHERE status = 'completed'
                           AND strftime('%Y', created_at) = ? AND strftime('%m', created_at) = ?`,
      [targetYear.toString(), targetMonth.toString().padStart(2, '0')]);
    ...
    res.json({
      ...
      completion_rate: totalTopics && Number(totalTopics.count) ? ((Number(completedTopics?.count) || 0) / Number(totalTopics.count) * 100).toFixed(1) : '0',
      overdue_rate: totalTopics && Number(totalTopics.count) ? ((Number(overdueTopics?.count) || 0) / Number(totalTopics.count) * 100).toFixed(1) : '0',
...
router.post('/', authenticate, requireRole(['admin', 'director']), async (req, res) => {
  try {
    ...
    const exists = await queryOne(`SELECT COUNT(*) as count FROM analytics WHERE topic_id = ? AND data_date = ?`, [topic_id, data_date]);
    
    if (Number(exists?.count) > 0) {
      await execute(`UPDATE analytics SET views = ?, likes = ?, shares = ?, comments = ? 
              WHERE topic_id = ? AND data_date = ?`,
        [views, likes, shares, comments, topic_id, data_date]);
    } else {
      await execute(`INSERT INTO analytics (topic_id, views, likes, shares, comments, data_date) 
              VALUES (?, ?, ?, ?, ?, ?)`,
