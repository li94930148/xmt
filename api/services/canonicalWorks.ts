...
const keyFor = (row: RecordValue) => {
  const title = normalizedTitle(row.title);
  const publishTime = published(row.publish_time);
  if (title && publishTime) return `published:${accountKey(row)}:${title}|${publishTime}`;
  return validId(row.aweme_id)
    ? `id:${accountKey(row)}:${text(row.aweme_id)}`
    : `unresolved:${accountKey(row)}:${Number(row.id) || text(row.aweme_id)}`;
};

export type CanonicalWork = DouyinMetricsWork & { cover_candidates: string[]; canonical_source_count: number; canonical_collision: boolean; canonical_platform_item_ids: string[] };
...
export function canonicalizeDouyinWorks(rows: DouyinMetricsWork[]): CanonicalWork[] {
  ...
  const safeGroups = [...groups.values()].map(group => ({ group, collision: group.length > 1 }));
  return safeGroups.map(({ group, collision }) => {
    ...
    return { ...representative, cover_url: covers[0] || '', cover_candidates: covers, canonical_source_count: group.length, canonical_collision: collision, canonical_platform_item_ids: ordered.map(row => text(row.aweme_id)).filter(Boolean) };
  });
}
