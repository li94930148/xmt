...
const keyFor = (row: RecordValue) => validId(row.aweme_id) ? `id:${text(row.aweme_id)}` : `fallback:${normalizedTitle(row.title)}|${published(row.publish_time)}`;
...
export function canonicalizeDouyinWorks(rows: DouyinMetricsWork[]): CanonicalWork[] {
  ...
  const safeGroups = [...groups.values()].flatMap(group => !validId(group[0].aweme_id) && group.length > 1 ? group.map(row => [row]) : [group]);
  return safeGroups.map(group => {
    const ordered = [...group].sort((left, right) => timestamp(right) - timestamp(left) || completeness(right) - completeness(left) || Number(right.id) - Number(left.id));
    const representative = ordered[0];
    const covers = [...new Set(ordered.map(row => resolveCoverUrl({ douyinCoverUrl: row.cover_url })).filter(Boolean))].slice(0, 4);
    return { ...representative, cover_url: covers[0] || '', cover_candidates: covers, canonical_source_count: group.length, canonical_collision: false };
  });
}
