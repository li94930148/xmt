...
export async function resolveCreatorAccountScope(userId: number, role: string, platformUid?: string): Promise<CreatorAccountScope | null> {
  if (role === 'admin') return queryOne<CreatorAccountScope>(`SELECT id,user_id,platform_uid,'manage' access_level FROM creator_platform_accounts ${platformUid ? 'WHERE platform_uid=?' : ''} ORDER BY updated_at DESC LIMIT 1`, platformUid ? [platformUid] : []);
  return queryOne<CreatorAccountScope>(`SELECT a.id,a.user_id,a.platform_uid,CASE WHEN a.user_id=? THEN 'manage' ELSE x.access_level END access_level FROM creator_platform_accounts a LEFT JOIN creator_account_access x ON x.account_id=a.id AND x.user_id=? WHERE (a.user_id=? OR x.user_id=?) ${platformUid ? 'AND a.platform_uid=?' : ''} ORDER BY a.updated_at DESC LIMIT 1`, platformUid ? [userId,userId,userId,userId,platformUid] : [userId,userId,userId,userId]);
}
