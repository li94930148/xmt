...
export async function persistNormalizedDouyinSync(agent: AgentIdentity, payload: JsonRecord, snapshotTime: string, taskId: string) {
  ...
  return runInTransaction(async tx => {
    ...
    await tx.execute(`INSERT INTO douyin_accounts(name,profile_url,douyin_id,user_id,nickname,avatar,douyin_uid,fans_count,fans_count_available,following_count,works_count,total_likes,last_sync_time,creator_account_id,updated_at)
      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)
      ON CONFLICT(douyin_uid) DO UPDATE SET user_id=excluded.user_id,nickname=CASE WHEN trim(excluded.nickname)<>'' THEN excluded.nickname ELSE douyin_accounts.nickname END,avatar=CASE WHEN trim(excluded.avatar)<>'' THEN excluded.avatar ELSE douyin_accounts.avatar END,fans_count=CASE WHEN excluded.fans_count_available=1 THEN excluded.fans_count ELSE douyin_accounts.fans_count END,fans_count_available=MAX(douyin_accounts.fans_count_available,excluded.fans_count_available),following_count=CASE WHEN ? THEN excluded.following_count ELSE douyin_accounts.following_count END,works_count=CASE WHEN ? THEN excluded.works_count ELSE douyin_accounts.works_count END,total_likes=CASE WHEN ? THEN excluded.total_likes ELSE douyin_accounts.total_likes END,last_sync_time=excluded.last_sync_time,creator_account_id=excluded.creator_account_id,updated_at=CURRENT_TIMESTAMP`,
      [normalized.account.nickname || normalized.account.douyin_uid, `https://www.douyin.com/user/${encodeURIComponent(normalized.account.douyin_uid)}`, normalized.account.douyin_uid, agent.user_id, normalized.account.nickname, normalized.account.avatar, normalized.account.douyin_uid, normalized.account.fans_count, Number(normalized.account.fans_count_available), normalized.account.following_count, normalized.account.works_count, normalized.account.total_likes, snapshotTime, creatorAccount.id, Number(normalized.account.metadata_observed.following_count), Number(normalized.account.metadata_observed.works_count), Number(normalized.account.metadata_observed.total_likes)]);
