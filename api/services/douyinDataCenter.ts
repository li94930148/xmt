...
import { douyinDataNormalizer, type NormalizedDouyinWork } from './douyinDataNormalizer.js';
...
export async function persistNormalizedDouyinSync(agent: AgentIdentity, payload: JsonRecord, snapshotTime: string, taskId: string) {
  ...
  return runInTransaction(async tx => {
    ...
    await tx.execute(`DELETE FROM creator_content_items WHERE lower(trim(title)) IN ('react','flash_mod_modal','start_flash_mod')`);
