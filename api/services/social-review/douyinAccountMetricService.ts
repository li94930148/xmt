...
export async function syncDouyinAccountMetrics(accountId: number, page: Page) {
  ...
  for (const [name, value] of [['followers', followerCount], ['video_count', awemeCount], ['total_favorited', totalFavorited], ['following_count', followingCount]] as const) if (value != null) { await execute(`INSERT INTO social_account_metric_snapshots(account_id,metric_name,metric_value,snapshot_date,source,created_at) VALUES(?,?,?,date('now','+8 hours'),'chrome_cdp',datetime('now','+8 hours')) ON CONFLICT(account_id,metric_name,snapshot_date) DO UPDATE SET metric_value=excluded.metric_value,source=excluded.source`, [accountId, name, value]); summary.accountMetricCount += 1; }
