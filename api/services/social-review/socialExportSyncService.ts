...
export async function syncLatestOfficialExport(accountId: number) {
  ...
  const reasonCounts = (parsed as unknown as { skipReasonCounts?: Record<string, number> } | undefined)?.skipReasonCounts || {};
  ...
  const skipped = Number(parsed?.skippedRowCount || 0);
  return {
    success: !result.errorMessage,
    accountId,
    totalRows: Number(parsed?.parsedRowCount || result.videoCount || 0),
