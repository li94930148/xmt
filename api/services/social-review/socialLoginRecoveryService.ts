...
export async function getSocialLoginRecovery(id: string) {
  ...
  if (['waiting_scan', 'scanned', 'manual_verify_required', 'auth_required'].includes(session.status)) { const login = await detectServerLogin(Number(session.account_id)); if (login === 'manual_verify_required') { await execute(`UPDATE social_login_sessions SET status='manual_verify_required',updated_at=datetime('now','+8 hours') WHERE id=?`, [id]); session.status = 'manual_verify_required'; } else if (login === 'logged_in') { await execute(`UPDATE social_login_sessions SET status='success',updated_at=datetime('now','+8 hours') WHERE id=?`, [id]); await execute(`UPDATE social_credentials SET status='active',last_success_time=datetime('now','+8 hours'),updated_at=datetime('now','+8 hours') WHERE account_id=?`, [session.account_id]); session.status = 'success'; } }
