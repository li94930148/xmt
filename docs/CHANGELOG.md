...
- 按钮、输入框、卡片、状态标签、空态/骨架/错误态走统一 token 与组件契约，业务页不再散落硬编码色。
- 命令面板、模态、帮助与更新弹窗共用同一套浮层入场编排（遮罩淡入 + 面板上浮）。
- 登录页三套布局对齐主强调色并提升对比度；首页驾驶舱浅色主题表面材质精修。
- 富文本编辑器工具栏/菜单 chrome 对齐 token，正文高亮色板保持独立语义。
- 新增 DESIGN.md 与 AGENTS.md，约定后续界面开发的设计系统调用关系。
...
- 运营概览区分官方周期新增播放、互动与粉丝净变化，并对每个缺失字段说明原因。

### 修复

- 官方作品导入不再把沿用的旧粉丝总量写成当天实测快照；同日已有真实观测仍保留。
...
- 官方导入契约、统一报告、周期计算、桌面和移动宽度页面渲染、类型检查与构建通过。
...
- 创作协作房间按稿件版本隔离，旧版本不能继续向新版本写入。

### 修复

- 普通输入或粘贴不再自动将 v2.3 升为 v3.0；只有明确选择另开新版才提升大版本。
- 多人协作时远端内容不会触发本地旧稿覆盖；保存前校验已持久化正文，冲突时提示而不静默丢稿。
- 内容动态将历史记录显示为旧版快照，保存时间按北京时间显示，操作人读取实际保存者。
...
- 创作版本、协作权限、保存冲突及时间轴回归测试通过；双账号线上登录和发布验收另行记录。

## v3.0.3 - 2026-09-21

### 修复

- Creator Agent v2.14.3-agent 的每日与 12 小时自动任务固定使用官方四文件采集路径，避免全量作品滚动中的浏览器上下文关闭阻断定时导出。
- 手动全量同步仍保留；自动任务继续执行作品列表、昨天、近 7 天、近 30 天的完整质量门校验和加密上传。

### 数据库变化

- 无迁移，不修改既有作品、官方日指标或同步历史。

## v3.0.2 - 2026-09-19

### 修复

- Creator Agent v2.14.2-agent 兼容抖音接口返回的对象形态作品状态，提取其中可验证的标量值；未知结构按缺失处理，不把对象写入 SQLite。
- 官方作品列表及昨天、近 7 天、近 30 天文件仍由原质量门槛校验，计算口径和最新数据优先规则不变。
...
- Creator Agent v2.14.1-agent 固定使用数据中心“作品数据”区域的导出按钮，不再误取页面下方的粉丝指标导出。
- 为导出按钮的 DOM 顺序增加回归测试；错源文件仍会因日期行数不符而拒绝上传。
...
- Creator Agent v2.14.0-agent 后台静默下载内容管理作品列表，以及数据中心昨天、近 7 天、近 30 天三份作品数据。
- 运营概览、数据趋势和日报/周报/月报接入官方逐日指标，保留每个统计周期的文件来源和采集时间。

### 优化

- 累计作品指标以最新作品列表为准；周期计数按日求和，比例和平均播放时长按播放量加权。
- 昨天、近 7 天、近 30 天严格选择对应文件，不跨周期拼接；90 天趋势使用滚动沉淀的官方日事实。

### 修复

- 日流量不再被当作累计快照进行二次差分；平台返回的负评论修正值不再被清零。
- 较旧采集批次不能覆盖较新官方指标；四份文件缺失任一项时拒绝残缺同步。

### 技术升级

- 官方导出解析器升级为 v3；自动采集强制无头 Chromium，并拒绝可见 external-CDP 会话。

### 数据库变化

- migration 014 新增官方账号日指标表，并在文件审计表记录数据集类型和来源周期；不删除、合并或回填既有业务历史。

### 测试情况

- 用户提供的官方样本完成 43 行作品和 30 行日指标解析；解析、计算、迁移、加密上传、幂等、统一报告和类型检查通过。真实账号四文件后台下载需在安装新版 Agent 后完成一次验收。

## v2.22.3 - 2026-09-17

### 修复

- 修复抖音官方累计指标与旧采集快照混算后产生虚假负增长的问题。
- 没有同口径官方期初数据时，播放和互动周期变化明确显示暂无数据。
...
- 官方导出中播放为零且没有单独播放指标行的作品，现在仍按最新官方文件中的作品键计入作品总数；累计播放和互动仍只汇总实际存在的官方指标。
...
- 无 schema 迁移，不补造缺失指标，不删除或合并历史记录。

### 测试情况

- 统一抖音报告合同新增“无播放指标行作品”回归覆盖；TypeScript、版本一致性与生产构建通过。
...
- 运营概览和复盘报告以最新一次抖音创作者中心官方作品导出为累计作品、播放、点赞、评论、分享和收藏的统一来源；页面标明官方数据更新时间。
- 官方作品仅在标题和发布日期完全一致时校准本地作品明细，无法确定的记录继续保留在官方汇总中并提示，不进行猜测关联。
- 周期变化只有在期初附近存在可比快照时才计算，避免跨越过长时间误算增长。

### 修复

- Creator Agent v2.13.12-agent 识别创作者中心实际粉丝字段，修复实时粉丝无法更新、回退到旧值的问题。
- 官方汇总限定为最新完整导出文件，避免已经不在本次导出中的旧作品继续参与累计。

### 技术升级

- 官方导出解析器升级为 v2，使既有官方文件可在升级后重新入队完成口径校准。

### 数据库变化

- 无 schema 迁移；不删除、合并或伪造既有作品、快照及同步历史。完全匹配的作品仅追加官方快照并更新当前指标。

### 测试情况

- 粉丝字段归一、官方导出事务与队列、最新导出口径、统一运营报告和 TypeScript 检查通过；生产构建与发布验证在部署闭环记录。

## v2.22.0 - 2026-09-16

### 新增

- 复盘报告新增删除操作；删除受报告管理权限和当前抖音账号范围双重约束，并提供不可恢复确认。
- 新版报告展示当前累计播放、当前粉丝、周期播放/粉丝/互动变化、作品分层、优秀/低效作品与数据完整度。

### 优化

- 抖音运营概览、作品、趋势和复盘报告统一读取 `douyin_*` 标准化数据；累计播放定义为去重作品当前播放量之和，周期增长定义为日快照期末减期初，不再读取长期为零的旧账号指标孤岛。
- 无可靠快照或采集源未提供粉丝指标时明确显示“暂无数据”，不再用 `0` 伪装完整数据。
- 删除没有可用生产数据来源的粉丝画像页面、导航和 API；保留真实粉丝总量及粉丝变化指标。
- 登录安全升级页只有在指标端点实际被外部读取后才显示抓取证据，并区分 Socket 独立审批与运行开关。

### 数据库变化

- 无 schema 迁移；不删除或改写既有作品、快照、报告和粉丝画像历史数据。页面仅下线不可用入口，旧报告仍可查看并由有权限用户逐份删除。

### 测试情况

- 新增统一报告合同测试，覆盖累计播放、快照增量、粉丝、旧指标隔离和跨账号删除保护；Auth 指标测试覆盖“指标事件不等于外部抓取”。TypeScript、版本一致性、生产构建，以及桌面/390px 移动端报告与认证页浏览器回归通过。

## v2.21.4 - 2026-09-16

### 新增

- 备份管理新增服务器多来源清单，覆盖应用内备份、部署应急备份、系统定时压缩备份和显式配置的附加目录。
- 登录安全升级页新增基于运行配置、外部指标、Socket Bridge 和风险窗口的下一阶段准入判断。

### 优化

- “内容时间轴”简化为“内容动态”，按编辑会话展示自然语言记录，并主动读取创作版本历史；最近内容标题会正确解析到真实文档，不再被当作伪文档编号。
- 抖音运营中心合并与作品库完全重复的“作品复盘”入口，统一为运营概览、作品管理、数据趋势、粉丝画像和复盘报告。

### 修复

- 修复备份页面只能发现数据库同级 `backups` 目录、看不到服务器定时和部署备份的问题。

### 技术升级

- 备份发现采用服务端目录白名单、文件名校验、普通文件校验与不暴露绝对路径的安全清单。
...
- CI 持续执行备份多来源发现和内容标题解析合同；本地 Auth 灰度治理与准入测试、TypeScript、定向 ESLint、版本一致性、生产构建和浏览器页面回归通过。
...
- 发布页顶部统计改为服务端按当前用户数据范围计算全量结果，不再只统计当前分页；“今日待发布”统一使用北京时间。
- 发布创建、编辑与分析录入使用事务保护同一批数据库写入，归档协调失败不再把已成功提交的发布操作伪装为失败。

### 修复

- 修复同一选题存在多日分析快照时发布列表重复、分页总数虚增，以及旧日期指标被批量覆盖的问题。
- 修复部分指标更新把未提交指标清零的问题。
- 修复抖音关联后的权威字段只在页面禁用、API 仍可绕过修改的问题。
- 修复分析数据录入缺少选题范围、日期及非负整数校验的问题。
...
- 无 schema 迁移；保留全部既有分析快照和发布记录，读取时确定性选择最新旧分析快照。

### 测试情况

- 发布—抖音关联、分析写入范围、API 合同、权限、安全、TypeScript、版本、构建及浏览器回归均纳入发布门禁。

## v2.21.0 - 2026-09-12

### 新增

- 发布管理支持刷新抖音关联、查看匹配候选、人工绑定和解除绑定；高置信且无歧义的标题会在抖音同步完成后自动关联。

### 优化

- 关联后的发布时间、发布状态和播放、点赞、分享、评论统一以抖音运营中心作品为准；发布稿件正文与原始排期字段继续独立保存。

### 安全与数据边界

- 候选查看要求 Creator 数据查看权限，修改关联同时要求发布管理权限和稿件编辑范围；未知服务异常只返回固定安全提示。
- 不按标题删除或合并历史记录；同一抖音作品只能关联一条发布记录，歧义结果不自动写入。

### 数据库变化

- migration 013 新增 `publishing_douyin_links` 一对一关联表及索引；不修改已有 `publishing`、`douyin_works` 或运营指标。

### 测试情况

- 标题归一化、歧义拒绝、自动/人工关联、解绑回退、抖音指标实时更新、权限/API、SQLite 外键与完整性专项回归通过。

## v2.20.25 - 2026-09-12

### 优化

- 首页任务、完成内容和生产指数改为聚合选题、创作、发布三段真实业务记录；生产指数使用各选题最深推进阶段的平均值。
- 首页播放量与抖音运营中心使用同一驾驶舱指标，并保留旧分析数据作为无权限或无账号时的回退。

### 修复

- 已审核稿件的创作资料不再触发富文本初始化、定时、重试或卸载补保存，避免连续出现只读状态保存失败提示。

### 数据库变化

- 无 schema 迁移，不修改已有稿件、资料、作品或运营数据。

### 测试情况

- 创作资料 API/UI 合同、首页聚合计算、TypeScript、版本治理和生产构建通过。
...
- 活动日志仅保留最近 7 天；升级时先清理过期记录，后续每次写入持续执行相同保留边界。
- 资料中心、日报归档、Social Review、Creator Center 与协作时间线统一通过共享北京时间层展示和解析。

### 修复

- 修复活动日志依赖 SQLite UTC 默认值、API 缺少时区偏移导致系统管理页面少显示 8 小时的问题。
- 修复运行时遗留 `CURRENT_TIMESTAMP` 写入 UTC，以及部分按周、按日、在线状态查询仍使用 UTC 边界的问题。

### 技术升级

- 日志 API 时间统一为带 `+08:00` 的 ISO-8601；运行时数据库兼容层统一绑定北京时间参数。

### 数据库变化

- migration 012 校正 `activity_log.created_at` 历史 UTC 值，删除 7 天前活动日志并创建持续保留触发器；新增不含操作详情的 `user_login_days` 日汇总，保证 7/30 天连续登录成就不受日志清理影响。

### 测试情况

- 活动日志时间/保留专项测试、时间合同、认证、Auth Web Cookie、选题、代码审查整改、TypeScript、版本治理与生产构建通过。

## v2.20.23 - 2026-09-11

### 新增

- 新增每篇创作唯一的资料草稿文档，支持连续输入、复制粘贴及按光标插入资料库正文。

### 优化

- 删除资料卡片、数量角标、单条编辑删除、“新建手动资料”和添加按钮二级菜单。
- 保留资料区高度拖动、双击复位和按登录用户隔离的本机偏好。

### 修复

- 空正文资料不会插入只有标题的空内容；多选中的其他有效正文仍按选择顺序插入。
- 统一草稿的串行自动保存与 revision 乐观锁阻止旧请求覆盖新内容。

### 技术升级

- 删除旧卡片 CRUD 和手动资料 API，资料库插入接口只返回经过安全转换的正文副本。

### 数据库变化

- migration 011 新增 `production_material_drafts`；按原排序幂等合并 `production_materials` 正文，不删除旧表或旧记录。

### 测试情况

- 覆盖历史卡片幂等合并、统一草稿保存、资料正文顺序、空正文、权限、跨用户越权、HTML 清洗、旧请求冲突、光标位置回退及高度用户隔离。

## v2.20.22 - 2026-09-10

### 新增

- 创作详情新增“创作资料”工作区，资料库内容以稿件级独立快照直接展示正文。
- 支持资料库多选添加、手动资料、富文本编辑、800ms 自动保存、逐条移除及可调高度。

### 优化

- 资料区高度按登录用户与当前设备隔离保存，支持拖动边界、键盘调整及双击复位。
- 已审核稿件、历史版本和无编辑权限状态统一只读，资料正文仍可选择复制。

### 修复

- 旧保存请求通过 revision 乐观锁返回冲突，不再覆盖更新后的资料内容。
- 删除资料快照仅移除当前创作关联，不删除资料中心原记录。

### 技术升级

- 增加服务端富文本白名单清洗，危险标签、事件属性和危险 URL 不写入数据库。

### 数据库变化

- 新增 `production_materials` 表、生产与排序索引、来源索引及非空来源唯一约束；历史 `resource_relations` 引用幂等回填为快照。

### 测试情况

- 新增资料快照、手动 CRUD、权限、跨创作、HTML 清洗、旧请求冲突、migration 幂等、SQLite 完整性及高度用户隔离测试。
