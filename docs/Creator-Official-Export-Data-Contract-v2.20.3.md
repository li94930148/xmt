...
> 历史契约：当前实现已由 `Creator-Official-Export-Data-Contract-v3.0.0.md` 取代。本文件仅保留 v2.20.3 当时的观察证据，不代表当前采集范围。
...
仪表盘读取模式由 `XMT_CREATOR_OFFICIAL_DASHBOARD_MODE` 白名单控制：默认 `official_preferred`；`existing_only` 可显式回退既有采集口径；`shadow_compare` 仍返回旧口径，只记录不含标题、昵称或原始指标值的对账状态。官方优先模式只汇总最近一次完整作品导出文件中的作品数、播放、点赞、评论、分享、收藏、主页访问和新增粉丝，不叠加更早导出中已不存在的作品；实时粉丝总数仍来自账号当前采集值。官方作品仅在规范化标题和发布日期完全一致时回写本地作品指标及快照，未匹配项保留在官方汇总并给出警告，不执行模糊覆盖、删除或合并。
