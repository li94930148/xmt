...
- 新增月度总结、年度总结、日报日历、团队统计与数据分析接口及页面。
- 日报模板支持个人归属与排序，补充模板管理权限；新增兼容迁移 006。
- 日报编辑器接入轻量 Tiptap 富文本和 30 秒自动保存，自动保存不递增版本、不写审计日志。
...
本次升级新增并执行数据库迁移 `006_daily_workspace_v2`，新增月度/年度总结表，并为日报模板补充归属与排序字段。现有日报、用户、会话和业务数据保持兼容，不改变既有提交、审核和审计链路。
