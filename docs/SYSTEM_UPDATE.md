...
创作详情页的“参考资料”已升级为“创作资料”工作区。加入资料库内容后，正文会直接显示在稿件上方；也可以新建手动资料，把网页、微信或文档中的文字粘贴进来继续整理。资料可单独编辑、自动保存和移除，移除不会删除资料中心原文。

资料区可拖动底部手柄调整高度，双击恢复默认高度，并按当前登录用户保存在这台设备。已审核稿件、历史版本或无编辑权限的用户保持只读，但仍可查看和复制资料正文。

系统会把既有参考资料关联安全转换为当前稿件的独立快照。后续资料中心原文变化或删除，不会使已加入稿件的资料正文无故消失。本次不改变正式稿件协同编辑、自动保存、版本、审核或时间轴。
