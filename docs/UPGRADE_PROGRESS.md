...
- 完成内容：改密、RBAC、资源与协作权限；旧运营入口收敛；番茄钟真实落库；选题删除事务化；Agent 空模块状态和有界采集桥接。
- 修改范围：认证、权限、资料、选题、协作、番茄钟、导航、Agent/Collector、测试及版本文档。
- 数据库变化：无新迁移；仅未完成的本人番茄钟可被放弃删除，选题删除失败会回滚。
- 测试结果：见 `releases/v3.1.0-project-integrity-remediation.md`，正式环境与真实账号采集仍需单独验收。
- 风险说明：审计报告的其他 P1/P2 项及官方四文件协议中的原始 XHR 上传未在本阶段完成；不应把本次桥接视作正式环境采集复验。
- 下一阶段：逐项核对剩余报告条目，优先补齐生产记录分页、服务端错误响应与官方导出关联；按发布门禁完成真实验收。
