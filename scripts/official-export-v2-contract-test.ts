...
const unchangedPayload={...payload,batch_id:crypto.randomUUID()}; const unchanged=await acceptCreatorDataSync(envelope(unchangedPayload),`Bearer ${token}`) as { result:{unchanged:number} }; assert.equal(unchanged.result.unchanged,3);
const refreshedSha='c'.repeat(64); const refreshedPayload={...payload,batch_id:crypto.randomUUID(),source_files:[{...payload.source_files[0],sha256:refreshedSha}]}; const refreshed=await acceptCreatorDataSync(envelope(refreshedPayload),`Bearer ${token}`) as { result:{unchanged:number} }; assert.equal(refreshed.result.unchanged,3);
