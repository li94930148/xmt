...
await assert.rejects(()=>acceptCreatorDataSync(envelope({...payload,batch_id:crypto.randomUUID(),source_files:[...payload.source_files,...payload.source_files]}),`Bearer ${token}`)); assert.equal((await queryOne<{count:number}>('SELECT COUNT(*) count FROM creator_ingest_batches'))?.count,2);
