...
export type DouyinAccount = { id: number; nickname?: string; open_id: string; avatar?: string; status: string; expires_at?: string; last_sync_at?: string; douyin_data_source?: string };
export type DouyinDashboard = { account:{id:number;nickname?:string;avatar?:string;open_id:string;douyin_data_source:string};trends:Array<{snapshot_date:string;play_count:number;new_fans:number;new_like_count:number;new_comment_count:number;profile_view_count:number;douyin_data_source:string}>;fansSources:Array<{snapshot_date:string;source_type:string;count:number;douyin_data_source:string}>;videoCapability:{available:false;status:string;message:string};sourcePriority:string[] };
...
export const syncDouyin = (account_id: number) => request<{success:boolean;results:Record<string,string>;warnings:string[]}>('/sync', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ account_id }) });
export const getDouyinDashboard = (account_id:number) => request<DouyinDashboard>(`/dashboard?account_id=${account_id}`);
