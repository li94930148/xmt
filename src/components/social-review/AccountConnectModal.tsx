...
export default function AccountConnectModal({ open, onClose, onSuccess }: Props) {
  ...
  return (
    <BaseModal
      ...
      footer={step === 1 ? (
        <div className="flex justify-end gap-3"><button type="button" onClick={onClose} className={`rounded-xl px-4 py-2 text-sm ${styles.buttonSecondary}`}>取消</button><button type="button" onClick={() => void start()} disabled={submitting} className="rounded-xl bg-gradient-to-r from-studio-primary to-studio-violet px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-60">{submitting ? '正在创建…' : '下一步 扫码登录'}</button></div>
      ) : (
        <div className="flex justify-end gap-3">{loginComplete ? <button type="button" onClick={() => onSuccess(accountId!)} className="rounded-xl bg-gradient-to-r from-studio-primary to-studio-violet px-4 py-2 text-sm font-medium text-white">进入账号复盘</button> : <button type="button" onClick={onClose} className={`rounded-xl px-4 py-2 text-sm ${styles.buttonSecondary}`}>{terminal ? '关闭' : '后台继续等待'}</button>}</div>
      )}
    >
      {step === 1 ? (
        <div className="space-y-5">
          ...
          {error ? <p className="text-sm text-studio-coral">{error}</p> : null}
        </div>
      ) : (
        <div className="space-y-4">
          <div className={`flex items-center gap-3 rounded-xl border px-4 py-3 ${styles.border}`}><StatusIcon className={`h-5 w-5 ${loginComplete ? 'text-studio-success' : terminal ? 'text-studio-coral' : 'animate-spin text-studio-primary'}`} /><div><p className={`font-medium ${styles.textPrimary}`}>{statusCopy[status]}</p><p className={`mt-0.5 text-sm ${styles.textMuted}`}>{nickname}</p></div></div>
          ...
          {error ? <p className="text-sm text-studio-coral">{error}</p> : null}
          {loginComplete ? <p className="text-sm text-studio-success">账号绑定成功。现在可以进入账号复盘页面。</p> : null}
        </div>
      )}
    </BaseModal>
  );
