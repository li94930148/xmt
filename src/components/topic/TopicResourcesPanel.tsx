        ...
        {loading ? <LoadingState type="table" rows={3} /> : loadError ? <ErrorState onRetry={() => void load()} /> : items.length === 0 ? <EmptyState title="暂无关联资料" /> : (
          <div className="overflow-hidden rounded-xl border border-studio-border-soft">
            <div className="divide-y divide-studio-border-soft">
              {items.map((item) => <div key={item.id} className="grid items-center gap-3 px-4 py-3 text-sm md:grid-cols-[minmax(0,1fr)_130px_160px_auto]"><Link to={`/asset-center/resources/${item.id}`} className="truncate font-medium text-studio-text-primary hover:text-studio-primary-contrast">{item.title}</Link><span className="text-studio-text-secondary">{libraryNames[item.library_type]}</span><span className="truncate text-studio-text-secondary">{item.category?.name || '—'}</span><div className="flex justify-end gap-1"><Link aria-label="查看资料" to={`/asset-center/resources/${item.id}`} className="rounded-lg p-2 text-studio-text-secondary hover:bg-studio-surface-soft hover:text-studio-primary-contrast"><ExternalLink className="h-4 w-4" /></Link>{canManage ? <button type="button" aria-label="解除关联" onClick={() => setRemoving(item)} className="rounded-lg p-2 text-studio-text-secondary hover:bg-studio-coral/10 hover:text-studio-coral-contrast"><Trash2 className="h-4 w-4" /></button> : null}</div></div>)}
            </div>
          </div>
        )}
      </div>
