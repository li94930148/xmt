...
import { ReactBitsHeadingSlot } from './slots/ReactBitsHeadingSlot';
import { ReactBitsMetricSlot } from './slots/ReactBitsMetricSlot';
import { ReactBitsNavigationSlot } from './slots/ReactBitsNavigationSlot';
import { ReactBitsPageScene } from './slots/ReactBitsPageScene';
...
const motionOptions = [['off', '关闭'], ['reduced', '低'], ['balanced', '平衡'], ['full', '完整']] as const;

function PreviewSurface({ theme }: { theme: 'light' | 'dark' }) {
  const light = theme === 'light';
  return <ReactBitsPageScene page="home" previewMode forceRender className={`min-h-[330px] overflow-hidden rounded-2xl border p-5 ${light ? 'border-slate-300 bg-slate-50 text-slate-900' : 'border-slate-700 bg-slate-950 text-white'}`} fallbackClassName={light ? 'bg-gradient-to-br from-sky-50 via-white to-indigo-100' : 'bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900'}>
    <div className="relative z-10"><p className={`text-xs font-semibold tracking-wider ${light ? 'text-blue-700' : 'text-cyan-200'}`}>{light ? '浅色真实预览' : '深色真实预览'}</p>
      <div className="mt-2 font-black"><ReactBitsHeadingSlot semantic="brand-title">XMT 内容生产驾驶舱</ReactBitsHeadingSlot></div>
      <ReactBitsNavigationSlot semantic="settings-tabs" aria-label="预览导航" className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-full px-2 py-1">概览</span><span className="rounded-full px-2 py-1">内容</span><span className="rounded-full px-2 py-1">分析</span></ReactBitsNavigationSlot>
      <ReactBitsRevealSlot className="mt-5"><ReactBitsCardSlot semantic="settings-panel" className={`max-w-md rounded-xl border p-4 backdrop-blur ${light ? 'border-slate-300 bg-white/85 text-slate-900' : 'border-white/15 bg-slate-950/55 text-white'}`}><p className={light ? 'text-slate-600' : 'text-slate-300'}>今日内容生产指数</p><p className="mt-2 text-4xl font-bold"><ReactBitsTextSlot semantic="metric" themeOverride={theme} value={86}>86</ReactBitsTextSlot><span>%</span></p><p className={`mt-2 text-xs ${light ? 'text-slate-600' : 'text-slate-400'}`}>同一草稿配置的主题兼容预览。</p></ReactBitsCardSlot></ReactBitsRevealSlot>
...
export function ReactBitsAppearancePanel() {
  ...
  const valueAt = (path: string) => path.split('.').reduce<unknown>((item, key) => (item as Record<string, unknown>)[key], draft);
  ...
  const pageLabels: Record<string, string> = { login: '登录页', home: '首页', topics: '选题', creator: 'Creator', analytics: 'Analytics', dailyReport: '日报', workflow: 'Workflow', editor: 'Editor' };
  return <section aria-labelledby="reactbits-appearance-title" className="mt-8 space-y-5 border-t border-studio-border-soft pt-6">
    <div><h3 id="reactbits-appearance-title" className="text-base font-semibold text-studio-text-primary">React Bits 原生动效外观中心</h3><p className="mt-1 text-sm text-studio-text-muted">只使用 React Bits 官方组件；修改先在下方实时预览，保存后才应用到系统。</p></div>
    <div role="region" aria-label="视觉方案预设"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{reactBitsPresets.map((preset) => <button key={preset.id} type="button" aria-pressed={draft.presetId === preset.id} onClick={() => applyPreset(preset.id)} className={`rounded-xl border p-4 text-left ${draft.presetId === preset.id ? 'border-studio-cyan bg-studio-cyan/10' : 'border-studio-border-soft bg-white/[0.03]'}`}><p className="font-semibold text-studio-text-primary">{preset.label}</p><p className="mt-1 text-xs text-studio-text-muted">{preset.description}</p></button>)}</div></div>
    <div><p className="mb-2 text-sm font-medium text-studio-text-secondary">页面应用范围</p><div className="flex flex-wrap gap-3">{Object.entries(draft.applyTo).map(([page, enabled]) => <label key={page} className="inline-flex items-center gap-2 rounded-lg border border-studio-border-soft px-3 py-2 text-xs text-studio-text-secondary"><input id={`appearance-apply-${page}`} aria-label={`应用到${pageLabels[page] || page}`} type="checkbox" checked={enabled} onChange={(event) => setDraftField(`applyTo.${page}`, event.target.checked)} />{pageLabels[page] || page}</label>)}</div></div>
    <label className="text-sm text-studio-text-secondary"><span className="mb-2 block font-medium">动效强度</span><select id="appearance-motion-mode" aria-label="动效强度" value={String(valueAt('motionMode'))} onChange={(event) => setDraftField('motionMode', event.target.value)} className="w-full rounded-xl border border-studio-border-soft bg-studio-app-bg px-3 py-2.5 text-studio-text-primary">{motionOptions.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></label>
    <details className="rounded-2xl border border-studio-border-soft p-4"><summary className="cursor-pointer text-sm font-semibold text-studio-text-primary">高级设置</summary><div className="mt-4 grid gap-4 md:grid-cols-2">{fields.map(([path, label, options]) => <label key={path} className="text-sm text-studio-text-secondary"><span className="mb-2 block font-medium">{label}</span><select id={`appearance-${path.replace('.', '-')}`} aria-label={label} value={String(valueAt(path))} onChange={(event) => setDraftField(path, event.target.value)} className="w-full rounded-xl border border-studio-border-soft bg-studio-app-bg px-3 py-2.5 text-studio-text-primary">{options.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></label>)}</div></details>
