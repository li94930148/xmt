...
export function ReactBitsAppearancePanel() {
  ...
  return <section className="mt-8 space-y-5 border-t border-white/10 pt-6">
    ...
    <ReactBitsAppearancePreviewProvider config={draft}><div className="relative isolate min-h-[310px] overflow-hidden rounded-2xl border border-studio-border-soft bg-slate-950 p-6"><ReactBitsBackgroundSlot page="home" fallbackClassName="bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900" /><div className="relative z-10"><p className="text-xs font-semibold tracking-wider text-cyan-200">实时预览</p><div className="mt-3 text-3xl font-black text-white"><ReactBitsTextSlot semantic="brand-title">XMT 内容生产驾驶舱</ReactBitsTextSlot></div><ReactBitsRevealSlot className="mt-6"><ReactBitsCardSlot className="max-w-md rounded-xl border border-white/15 bg-slate-950/55 p-4 backdrop-blur"><p className="text-sm text-slate-300">今日内容生产指数</p><p className="mt-2 text-4xl font-bold text-white"><ReactBitsTextSlot semantic="metric" value={86}>86</ReactBitsTextSlot><span>%</span></p><p className="mt-2 text-xs text-slate-400">官方组件会根据当前草稿即时预览。</p></ReactBitsCardSlot></ReactBitsRevealSlot><div className="mt-5 flex flex-wrap gap-3"><ReactBitsButtonSlot variant="primary" onClick={() => undefined}>主要操作</ReactBitsButtonSlot><ReactBitsButtonSlot variant="secondary" onClick={() => undefined}>次要操作</ReactBitsButtonSlot></div></div></div></ReactBitsAppearancePreviewProvider>
