...
export default function SocialReviewVideoDetail() {
  ...
  const metricCards = [
    { label: '播放量', value: count(video.views), icon: Eye, color: 'text-studio-cyan' },
    { label: '点赞', value: count(video.likes), icon: Heart, color: 'text-studio-coral' },
    { label: '评论', value: count(video.comments), icon: MessageCircle, color: 'text-studio-violet' },
    { label: '分享', value: count(video.shares), icon: Share2, color: 'text-studio-success' },
    { label: '收藏', value: count(video.collects), icon: Star, color: 'text-studio-amber' },
  ];
