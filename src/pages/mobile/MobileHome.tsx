...
const quickActions = [
  { label: '提报选题', path: '/topics/add', icon: FilePlus2 },
  { label: '写日报', path: '/daily-report', icon: PenLine },
...
export default function MobileHome() {
  ...
  return <div className="space-y-5">
    ...
    <section><h2 className="mb-3 text-base font-semibold">快捷操作</h2><div className="grid grid-cols-2 gap-3">{quickActions.map((action) => { const Icon = action.icon; return <button key={action.path} onClick={() => navigate(action.path)} className="flex min-h-24 flex-col justify-between rounded-2xl border border-studio-border-soft bg-studio-surface p-4 text-left"><Icon className="h-5 w-5 text-studio-cyan" /><span className="text-sm font-medium">{action.label}</span></button>; })}</div></section>
