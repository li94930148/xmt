...
export default function MobileMe() {
  ...
  return <div className="space-y-4">
    ...
    <section className="overflow-hidden rounded-2xl border border-studio-border-soft bg-studio-surface"><button type="button" onClick={() => setShowPassword((value) => !value)} className="flex min-h-14 w-full items-center justify-between px-4 text-sm"><span className="inline-flex items-center gap-3"><Lock className="h-5 w-5" />账号安全</span><ChevronRight className="h-5 w-5 text-studio-text-muted" /></button>{showPassword ? <div className="space-y-3 border-t border-studio-border-soft p-4"><input value={oldPassword} onChange={(event) => setOldPassword(event.target.value)} type="password" autoComplete="current-password" placeholder="当前密码" className="min-h-11 w-full rounded-xl border border-studio-border-soft bg-transparent px-3" /><input value={newPassword} onChange={(event) => setNewPassword(event.target.value)} type="password" autoComplete="new-password" placeholder="新密码（至少 6 位）" className="min-h-11 w-full rounded-xl border border-studio-border-soft bg-transparent px-3" /><button type="button" onClick={() => void savePassword()} className="min-h-11 rounded-xl bg-studio-primary px-4 text-sm text-white">更新密码</button>{passwordNotice ? <p role="status" className="text-sm text-studio-text-secondary">{passwordNotice}</p> : null}</div> : null}<button type="button" onClick={() => setShowChangelog((value) => !value)} className={row}><span className="inline-flex items-center gap-3"><History className="h-5 w-5" />更新说明</span><ChevronRight className="h-5 w-5 text-studio-text-muted" /></button>{showChangelog ? <div className="space-y-3 border-t border-studio-border-soft p-4">{changelog.slice(0, 3).map((entry) => <article key={entry.version}><p className="text-sm font-medium">v{entry.version} · {entry.title}</p><p className="mt-1 text-xs text-studio-text-muted">{entry.date}</p><ul className="mt-2 list-disc space-y-1 pl-4 text-sm text-studio-text-secondary">{entry.changes.slice(0, 4).map((change) => <li key={change.description}>{change.description}</li>)}</ul></article>)}</div> : null}<button type="button" onClick={() => navigate('/anonymous-feedback')} className={row}><span>反馈</span><ChevronRight className="h-5 w-5 text-studio-text-muted" /></button>{hasPermission('system:settings') ? <button type="button" onClick={() => navigate('/notification-settings')} className={row}><span>管理工具</span><ChevronRight className="h-5 w-5 text-studio-text-muted" /></button> : null}</section>
    <p className="px-1 text-xs text-studio-text-muted">XMT Mobile v{__APP_VERSION__}</p>
  </div>;
}
