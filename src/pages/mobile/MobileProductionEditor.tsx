...
export default function MobileProductionEditor() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (id && id !== 'new') { const result = await getProductionById(Number(id)); persistedContentRef.current = result.content || ''; setProduction(result); setTopicId(String(result.topic_id)); setContent(readSafeDraftValue<string>(draftKey) ?? result.content ?? ''); }
