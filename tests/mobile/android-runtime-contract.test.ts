...
assert.match(buildGradle, /versionCode\s+21910/);
assert.match(buildGradle, /versionName\s+"2\.19\.10"/);
...
assert.match(app, /NativeSessionBootstrap/);
assert.match(app, /installNativeAuthRuntime/);
assert.match(app, /refreshNativeSession/);
assert.match(app, /NativeSessionBootstrap>[\s\S]*<Routes>/);
