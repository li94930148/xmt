...
const save = (body: Record<string, unknown>) => fetch(url, {
  method: 'PUT',
...
try {
  ...
  assert.equal((await save({ content: '普通粘贴后的正文', expected_content: '原稿', version_action: 'none' })).status, 200, 'same merged content is idempotent');

  assert.equal((await save({ content: '旧页面内容', expected_content: '原稿' })).status, 409);
  assert.deepEqual(await current(), { version: 'v1.0', content: '普通粘贴后的正文' });

  assert.equal((await save({ content: '正式新版本', expected_content: '普通粘贴后的正文', version_action: 'major' })).status, 200);
